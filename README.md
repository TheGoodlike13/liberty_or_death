...
Next we must update `/etc/resolv.conf` again. We could've done it before, no?
I'm not sure if I'm supposed to replace the existing file or just add the lines:

    search goodlike.eu
    nameserver 192.168.1.7

Based on these [examples](https://www.shellhacks.com/setup-dns-resolution-resolvconf-example/)
I decided to keep `8.8.8.8` in my file and append the other two to the top.

The `search` part just helps with hostnames, so it doesn't do much.
I assume the `nameserver` has some ordering, probably matching file,
and we likely wanna check our own server first, at least from Samba.
...
Still no idea what it is, but optional is optional.

> Configuring Kerberos

Now that's something I should understand! :D

Looks like we need a `krb5.conf` file in `/etc` directory.
We're suggested to copy the file generated by Samba. Why not?

    sudo cp /usr/local/samba/private/krb5.conf /etc/krb5.conf
    
> cp: cannot stat '/usr/local/samba/private/krb5.conf': No such file or directory

Of course. Foolish of me to think random commands would "just work".

Looking back at the output vomit, I can just about find the file:

    sudo cp /var/lib/samba/private/krb5.conf /etc/krb5.conf

This time it works.

> Do not create a symbolic link to the the generated krb5.conf file.

No worries. I have no idea how to create symbolic links. All good!

> Testing your Samba AD DC

    sudo samba
    
Nothing happens, which usually means something happened, but I wasn't told what.

> If you installed Samba using packages, use the script or service configuration file
> included in the package to start Samba.

Uh, but I thought `sudo samba` started it? No?
I'll pretend it did and this guide is just having a stroke again.

> Verifying the File Server

All right! That's what I'm talking about. Verification is the best.

It seems to depend on the version of Samba. I've no idea what the version is.
We installed it via `apt`, which may have told me something back then,
but that information is lost to the aether long ago.

I'm just gonna assume it's a reasonably fresh version.

    sudo smbclient -L localhost -N
    
> sudo: smbclient: command not found

Well, shit. The command used in older versions is the same too.

I take some [#yummy_advice](#yummy_advice) and install the client:

    sudo apt install samba-client
    
This rightfully installs `smbclient`. We good now?

> session setup failed: NT_STATUS_INVALID_SID

Of course not.

[#nuking_tendency](#nuking_tendency) does not look promising.
Well, it does suggest using `kinit` first, which might help?

    sudo kinit administrator@GOODLIKE.EU
    
> kinit: Cannot find KDC for realm "GOODLIKE.EU" while getting initial credentials

Interesting. Let's review the generated `/etc/krb5.conf`:

> dns_lookup_kdc = true
>
> default_domain = goodlike.eu

Based on these two excerpts I guess that Kerberos is trying to contact
the ticket server via `goodlike.eu`, but the DNS lookup for that fails.

We've set the DNS to our own IP address (with google as backup), so...
Our own DNS server, which is supposed to be managed by Samba is down?
...
All works. I can only guess there was an issue with `smbclient` previously
